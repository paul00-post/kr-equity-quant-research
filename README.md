...
Net of realistic transaction costs (commission + Korean securities transaction tax + slippage, ~0.41% round-trip). "KOSPI200 buy & hold" is the KODEX 200 ETF (069500) price series, and both sides are price returns with no dividends: the strategy's price data drops on ex-dividend dates (checked on year-end dividend dates for a dozen high-yield stocks) and no dividend cash is credited, so the comparison is like-for-like — a total-return benchmark would look somewhat better than the figure shown. See also [Limitations](#limitations) — in particular, the most recent fold (2026) underperforms buy-and-hold by a wide margin, and the walk-forward selection itself turns out to be sensitive to how many candidate parameter sets are tried.
